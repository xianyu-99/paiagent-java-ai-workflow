...
1. **One active task only** â€?never work on multiple tasks simultaneously.
2. **One execution thread only** â€?no parallel dispatch.
3. **No parallel coding subagents** â€?Antigravity does not have `Task(...)`.
...
- Failures are related (fix one might fix others) â€?investigate together first
...
Each domain is independent â€?fixing authentication doesn't affect validation tests.
...
task_boundary:
  ...
  prompt: |
    ## Task Description
    [FULL TEXT of task from plan â€?paste it here]
...
- **`./implementer-prompt.md`** â€?Template for implementation task boundaries
- **`./spec-reviewer-prompt.md`** â€?Template for spec compliance review (did we build what was requested?)
- **`./code-quality-reviewer-prompt.md`** â€?Template for code quality review (is it well-built?)
...
- **What changed** â€?files modified, features implemented
- **What verification ran** â€?test results, lint results
- **What remains** â€?remaining tasks, known issues
...
- **Bad:** "Fix all the tests" â€?loses focus
- **Good:** "Fix user-auth.test.ts failures" â€?clear scope

**Context:**

- **Bad:** "Fix the validation bug" â€?unclear where
- **Good:** Paste error messages, test names, relevant code paths

**Constraints:**

- **Bad:** No constraints â€?task might refactor everything
- **Good:** "Only modify src/auth/ directory"

**Output:**

- **Bad:** "Fix it" â€?no visibility into what changed
- **Good:** "Report: root cause, changes made, test results"

**Reviews:**

- **Bad:** "It works, move on" â€?quality debt
...
Spec review: Spec compliant â€?all requirements met, nothing extra
...
- **using-git-worktrees** â€?Set up isolated workspace before starting
- **writing-plans** â€?Creates the plan this skill executes
- **requesting-code-review** â€?Code review template for quality reviews
- **finishing-a-development-branch** â€?Complete development after all tasks

**Should also use:**

- **test-driven-development** â€?Follow TDD for each task
- **verification-before-completion** â€?Final verification checklist

**Alternative workflow:**

- **executing-plans** â€?Use for worktree-based parallel session execution
