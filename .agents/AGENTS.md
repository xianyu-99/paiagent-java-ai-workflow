...
1. Prefer local skills in `.agents/skills/<skill-name>/SKILL.md`.
...
- `Skill` tool -> `view_file ~/.gemini/skills/<skill-name>/SKILL.md` (or project-local `.agents/skills/<skill-name>/SKILL.md`)
...
- First preference: project skills at `.agents/skills`.
- Second preference: user skills at `~/.gemini/skills`.
- If both exist, project-local skills win for this profile.
- Optional parity assets may exist at `.agents/workflows/*` and `.agents/agents/*` as entrypoint shims/reference profiles.
